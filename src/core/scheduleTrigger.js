...
//   logger.info(`[Schedule ${scheduleId}] ⏰ Kích hoạt task tự động...`);
...
//       logger.info(`[Schedule ${scheduleId}] Lịch không khả dụng. Bỏ qua.`);
//       return;
//     }

//     // 2. Chuẩn bị dữ liệu Task
//     const taskData = {
//       ...schedule.taskData, // (taskType, params được lưu trong JSON)
      
//       // Override thông tin chạy tự động
//       runType: "SCHEDULED", 
//       description: `[Auto] ${schedule.name} - ${new Date().toLocaleString('vi-VN')}`,
      
//       // [QUAN TRỌNG] Truyền ID vào đây để initiateTask lưu lại
//       scheduleId: schedule.id 
//     };

//     // 3. Khởi tạo Task
//     await taskService.initiateTask(userId, taskData);

//     // 4. Thực thi Task (Đẩy sang worker chính)
//     await taskService.executeTask(userId);
    
//     logger.info(`[Schedule ${scheduleId}] Đã đẩy task sang hàng đợi xử lý.`);
